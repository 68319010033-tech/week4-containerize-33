...
const stats = computed(() => ({
  ...
  totalValue: products.value.reduce(
    ...
    <main class="main">
      <div class="stats-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-body">
            <div class="stat-val" style="color: #dc2626">
              {{ stats.lowStock }}
            </div>
      ...
      <div class="state-box" v-else-if="filtered.length === 0">
        ...
        <div class="state-desc">
          {{
            search || catFilter
              ? "ลองเปลี่ยน keyword หรือ filter"
      ...
      <div class="product-grid" v-else>
        <div
          ...
          class="product-card"
          :class="{
            'card-low': p.stock > 0 && p.stock < 10,
            'card-out': p.stock <= 0,
          }"
        >
          <div class="card-top">
            <span class="cat-badge" :class="catClass(p.category)">{{
              p.category
            }}</span>
            <div class="product-name">{{ p.name }}</div>
            <div class="product-desc" v-if="p.description">
              {{ p.description }}
            </div>
            <div class="product-price">
              ฿{{
                parseFloat(p.price).toLocaleString("th-TH", {
      ...
      <div class="footer-container">
        <div class="footer-brand">
          ...
          <p class="footer-tagline">
            ระบบจัดการสินค้าคงคลังและแดชบอร์ดอัจฉริยะ
          </p>
        </div>
        ...
        <div class="footer-status">
          <div class="status-indicator">
            ...
            <span class="status-text">ระบบทำงานปกติ (API Connected)</span>
          </div>
        </div>

        <div class="footer-credit">
          <p>&copy; 2026 StockPro. All rights reserved.</p>
          <p class="developer">
            ชั้นปี <span class="dev-info">ปวส.2/3</span> เลขที่
            <span class="dev-info">033</span> Developed by
            <span class="dev-name">อชิรวิชญ์ พวงพิลา</span>
          </p>
        </div>
      </div>
    </footer>
    ...
    >
      <div class="modal confirm">
        ...
        <div class="confirm-desc">
          คุณต้องการลบ
          <strong>{{ confirmDelete?.name }}</strong> ออกจากระบบหรือไม่?<br />
          <span style="color: #dc2626">การกระทำนี้ไม่สามารถย้อนกลับได้</span>
        </div>
        <div class="confirm-actions">
          <button class="btn-cancel" @click="confirmDelete = null">
            ยกเลิก
          </button>
          <button
            class="btn-danger-confirm"
            @click="deleteProduct(confirmDelete.id)"
          >
            ลบเลย
          </button>
        </div>
...
.footer-credit .dev-info {
  color: #2C5EAD; /* สีน้ำเงินเดียวกับโลโก้หลัก */
  font-weight: 600;
